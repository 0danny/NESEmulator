...
using PPURegisterCallback = std::function<void(uint16_t address, uint16_t value, bool reading)>;
...
namespace Emulation
{
	...
	{
	private:
		PPURegisterCallback ppuRegisterCallback;
	...
	public:
		...
		void RegisterPPURegisterCallback(PPURegisterCallback callback);
	};
}
